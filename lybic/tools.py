...
class ComputerUse:
    ...
    async def auto_grounding(self, sandbox_id: str, model_message: str,
                             backend_provider: str = 'opencua-7b') -> dto.ComputerUseActionDto:
        """Convert the output of a planner model into actual execution actions
        On the backend, we translate your natural language input (this can also be text generated by LLM) and
        screenshots (obtained via sandbox_id) into dto.ComputerUseActionDto objects,
        which you can then call execute_computer_use_action to execute.

        For the selection of grounding models, we provide two grounding models for you to choose:
        'opencua-7b' and 'qwen2.5-vl-7b', both of which can easily process user input and screenshots and convert them
        into accurate coordinates.

        ## example
        model_message: `Open the chrome app in the desktop`, `click('Search box')`, `double-click('windows icon')`, `type("text")`

        Args:
            sandbox_id: the sandbox id lybic will get the screenshot and send to grounding model
            model_message: The LLM output or user input
            backend_provider: grounding_model,support: 'opencua-7b'(default) and 'qwen2.5-vl-7b'
