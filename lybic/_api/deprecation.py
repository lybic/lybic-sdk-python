"""Helper functions for deprecating parts of the Lybic API.
...
from lybic._api.internal import is_caller_internal


class LybicDeprecationWarning(DeprecationWarning):
    """A class for issuing deprecation warnings for Lybic users."""


class LybicPendingDeprecationWarning(PendingDeprecationWarning):
    """A class for issuing deprecation warnings for Lybic users."""
    ...
    *,
    ...
    def deprecate(
        ...
        *,
        ...
        if removal:
            if removal.startswith("1.") and package and package.startswith("lybic"):
...
def suppress_lybic_deprecation_warning() -> Generator[None, None, None]:
    """Context manager to suppress LybicDeprecationWarning."""
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", LybicDeprecationWarning)
        warnings.simplefilter("ignore", LybicPendingDeprecationWarning)
        yield
...
def warn_deprecated(
    ...
    *,
    ...
    warning_cls = (
        LybicPendingDeprecationWarning if pending else LybicDeprecationWarning
    )
    warning = warning_cls(message)
    warnings.warn(warning, category=LybicDeprecationWarning, stacklevel=4)


def surface_lybic_deprecation_warnings() -> None:
    """Unmute Lybic deprecation warnings."""
    warnings.filterwarnings(
        "default",
        category=LybicPendingDeprecationWarning,
    )

    warnings.filterwarnings(
        "default",
        category=LybicDeprecationWarning,
    )
